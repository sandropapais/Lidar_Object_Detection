...
void clusterHelper(int index, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
	...
	// find nearest points within distance tolerance
	std::vector<int> nearest = tree->search(points[index], distanceTol);

	// for each nearby point call cluster helper again if not processed
	for(int id : nearest)
	{
		if(!processed[id])
		{
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	while(i < points.size())
	{
		...
		// if not processed create a cluster and call helper
